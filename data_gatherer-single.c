...
//	printf("Loop iteration %d\n", i);
	num_matrices = i+1;
	A[i] = (float*) malloc(m*k*n*sizeof(float));
	B[i] = (float*) malloc(m*k*n*sizeof(float));
	C[i] = (float*) malloc(m*k*n*sizeof(float));
//	printf("Malloc done.\n");
...
int main(int argc, char **argv) {
  ...
  cblas_sgemm(CblasColMajor,CblasNoTrans,CblasNoTrans, m,n,k, -3.0*DBL_EPSILON*n, A,m, B,k, 1, C,m);
  for(i = 0; i < m*n; i++) {
    if(C[i] > 0) {
